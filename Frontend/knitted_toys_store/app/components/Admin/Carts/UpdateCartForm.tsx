...
import { reduceQuantityItem, removeFromCart, updateCartAdmin } from "@/app/services/Admin/serviceCartsAdmin";
...
export const UpdateCartForm = ({ cart, onSuccess }: Props) => {
  ...
  return (
    <Form layout="vertical" form={form}>
      ...
      {cart.cartItemsResponses?.length ? (
        cart.cartItemsResponses.map((item) => (
          <div key={item.toyId} style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
            ...
            <Button type="link" danger onClick={() => handleRemoveItemFromCart(item.toyId)}>Удалить</Button>
          </div>
        ))
      ) : (
        <p>Корзина пуста</p>
      ...
      <Form.Item>
        <Select placeholder="Выберите игрушку" style={{ width: "60%" }} value={selectedToy} onChange={(value) => setSelectedToy(value)}>
          {toys?.map((toy) => (
        ...
        <Button type="primary" onClick={handleAddToy}>Добавить в корзину</Button>
      </Form.Item>
    </Form>
  );
};
