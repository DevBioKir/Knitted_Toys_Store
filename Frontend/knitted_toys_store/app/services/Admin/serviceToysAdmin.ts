...
export const getToyByIdAdmin = async (id: string): Promise<ToyResponse> => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_DEV_API_BASE_URL}/AdminToy/${id}`,
    {
      method: "GET",
    }
  );
...
export const createToyAdmin = async (toyRequest: ToyRequest) => {
  try {
    await adminAPI.post("/AdminToy", toyRequest);
  } catch (err) {
...
export const updateToyAdmin = async (id: string, toyRequest: ToyRequest) => {
  try {
    await adminAPI.put(`/AdminToy/${id}`, toyRequest);
  } catch (err) {
...
export const deleteToyAdmin = async (id: string) => {
  try {
    await adminAPI.delete(`/AdminToy/${id}`);
  } catch (err) {
...
//     const response = await fetch(
//       `${process.env.NEXT_PUBLIC_DEV_API_BASE_URL}/ImageUpload/upload`,
//       {
//         method: "POST",
//         body: formData,
//         credentials: "include",
...
export async function uploadImage(file: File): Promise<string> {
  ...
  const formData = new FormData();
  formData.append("image", file); // Ключ должен совпадать с параметром в контроллере

  try {
    const response = await adminAPI.post("/ImageUpload/upload", formData, {
      headers: {
        "Content-Type": "multipart/form-data", // Указываем тип контента для загрузки файлов
      },
    });

    return response.data.filePath; // Возвращаем путь к файлу
  } catch (err) {
